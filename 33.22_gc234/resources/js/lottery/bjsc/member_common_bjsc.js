...
function getCountdown() {
	$.ajax({
		url : "/bjsc/countdown.do?timeStamp=" + new Date().getTime(),
		type : "get",
		dataType : "json",
		success : function(b) {
			if (0 == b.code) {
...
$('button#btn-orders-submit').bind("click", function() {
	...
	var xuanhaopd = $("#xuanhaopd").val();
	if (xuanhaopd == 1) {
		if (!is) {
			parent.Showbo.Msg.alert('请选择足够号码!');
			return false;
		}
	}
	...
	if (betGold > 0) {
		parent.Showbo.Msg.confirm("投注金额：" + betGold, function($el) {
			...
			if ($el == 'yes') {
				...
				$.ajax({
					url : "/bjsc/order.do?timeStamp=" + new Date().getTime(),
					data : Data,
					type : "post",
					dataType : "json",
					success : function(b) {
