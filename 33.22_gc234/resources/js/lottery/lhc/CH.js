...
//获取彩池
var itemInfo = "";
$.ajax({url: "/lhc/getItem.do?timeStamp=" + new Date().getTime()+"&poolId=12",
	type: "get",
	dataType: "json",
    success: function(b) {
    	itemInfo = b;
...
//为radio绑定单击事件
$("#ch_game").on("click",".round-table #table1 input[name='rtype']",function(event) {
	//取消checkbox禁用
	$("#ch_game .round-table #table2 input[name='num[]']").attr("disabled",false);//全部取消禁用
...
function resetcheckbox() {
	var n = $("#ch_game .round-table #table2 input[name='num[]']");
...
//为checkbox绑定单击事件
$("#ch_game").on("click",".round-table #table2 input[name='num[]']",function(e) {
	
	$('#ch_msgbox .PlayType').html('');
	$('#ch_msgbox').hide();
	$('#ch_msgbox #gold').val('');
	
	var resArray= new ArrayList();
	var n = $("#ch_game .round-table #table2 input[name='num[]']");
	var radio = $('#ch_game .round-table #table1');
...
//绑定下注事件
$('#ch_msgbox .submit_cen').bind("click", function() {

	var betGold = $("#ch_msgbox #gold").val();
...
function reset(){
	var n = $("#ch_game .round-table #table2 input[name='num[]']");
	var radio = $('#ch_game .round-table #table1').find("input[name='rtype']:checked");
	for(var j=0, t = n.length; j < t; j++){
		if(n[j].checked){
...
//清除投注
$("#ch_game").on('click',".round-table #table2 input[name='btnReset']",function(e){
	reset();
});

//清除投注
$("#ch_msgbox").on('click','input[name=btnCancel]',function(e){
	reset();
});

//投注列表
$("#ch_game").on('click',".round-table #table2 input[name='btnSubmit']",function(e){
	var resArray= new ArrayList();
	...
	var code = $('#ch_game #table1 tr:eq('+ (index+1) +') td:eq('+cellindex+')').text();
	...
	$('#ch_msgbox .PlayType').html(htmlstr);
	$('#ch_msgbox').show();
});