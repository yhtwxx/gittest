    ...
    _class.prototype = {
        render: function(){
            var ui = this,
                items = ui.items,
                select = ui.select,
                type = ':GROUP:6:RANGE:' + ui.range;

            var data = makeArray(select);
            var nums = data.length;
            var sumOfOdds = 0, odds = 0, cnt = 0, info = '', bets;

            if (nums >= ui.limit) {
                $.each(items, function(num, $el){
                    select.hasOwnProperty(num) ? $el.removeClass('disabled') : $el.addClass('disabled');
                });
            } else {
                $.each(items, function(num, $el){
        ...
        getBets: function(){
            ...
            if (this.range) {
                var odds = {};
                var type = 'GROUP:6:RANGE:' + this.range;
                odds[type] = this.odds();
