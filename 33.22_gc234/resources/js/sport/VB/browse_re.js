...
var pg=0;
var sel_league='';
var uid='skq8xj1km13706722l6068564';
var loading = 'Y';
var loading_var = 'Y';
var ShowType = '';
var ltype = '3';
...
function ShowGameList(){
	parent.header.chg_button_bg('VB','rb');
 ...
 if (top.hot_game==""){	
	...
	if (ObjDataFT[gamerec].isMaster=="Y"){
			...
			if(ObjDataFT[gamerec].nowServer==1){
				onelayer=onelayer.replace("*NOWC*","class=\'bm_dot_on\'");
			}
			if(ObjDataFT[gamerec].nowServer==0){
				onelayer=onelayer.replace("*NOWH*","class=\'bm_dot_on\'");
			}
			onelayer=onelayer.replace("*NOWH*","class=\'bm_dot_out\'");
			onelayer=onelayer.replace("*NOWC*","class=\'bm_dot_out\'");
			//onelayer=onelayer.replace("*GAMEH*",ObjDataFT[gamerec].scoreGameH);
			//onelayer=onelayer.replace("*GAMEC*",ObjDataFT[gamerec].scoreGameC);
			onelayer=onelayer.replace("*SETH*",ObjDataFT[gamerec].scorePointH);
			onelayer=onelayer.replace("*SETC*",ObjDataFT[gamerec].scorePointC);
			onelayer=onelayer.replace("*POINTH*",(ObjDataFT[gamerec].scoreSetH != 'null')?ObjDataFT[gamerec].scoreSetH:"0");
			onelayer=onelayer.replace("*POINTC*",(ObjDataFT[gamerec].scoreSetC != 'null')?ObjDataFT[gamerec].scoreSetC:"0");
	}else{
	...
	//onelayer = onelayer.replace("*MORE*",parseAllBets(ObjDataFT[gamerec]));  //2014.08羽毛球多玩法
...
function parseUrl(uid,odd_f_type,betTeam,GameData,gamerec,wtype){
	...
	urlArray['R']=new Array("../VB_order/VB_order_re.php".replace(reg,ballType),eval("GameData.team_"+betTeam.toLowerCase()));
	urlArray['OU']=new Array("../VB_order/VB_order_rou.php".replace(reg,ballType),(betTeam=="C" ? top.strOver : top.strUnder));
	urlArray['EO']=new Array("../VB_order/VB_order_rt.php".replace(reg,ballType), (betTeam=="O"  ? top.str_o : top.str_e));
	urlArray['M']=new Array("../VB_order/VB_order_m.php".replace(reg,ballType),(betTeam=="N" ? top.str_irish_kiss : eval("GameData.team_"+betTeam.toLowerCase())));
	urlArray['OUH'] = new Array("../VB_order/VB_order_rouhc.php".replace(reg,ballType),(betTeam=="O"?top.strOver:top.strUnder));
	urlArray['OUC'] = new Array("../VB_order/VB_order_rouhc.php".replace(reg,ballType),(betTeam=="O"?top.strOver:top.strUnder));
	...
	var ret="<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('"+ballType+"','"+rewtype[wtype]+"','"+param+"');\" title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+">"+ioratio+"</font></a>";
...
function show_allbets(gid,evt,datetime){
	...
	body_browse.document.getElementById('MVB').className="more_bar";
	...
	var  url="body_var_re_allbets.php?gid="+gid+"&uid="+uid+"&ltype="+ltype+"&langx="+top.langx+"&gtype=VB&date="+datetime;
	body_browse.showdata.location.href = url;
...
function onLoad(){
	...
	if (""+top.VB_RE_session=="undefined"){
		top.VB_RE_session="all";	
	}
	/* //sel_rtype
	...
	if(rtype == "p3"){
		...
		*/
		...
		if(Level == "up"){
			...
			if (tmp_sel_gtype=="VU"||tmp_sel_gtype=="TU"||tmp_sel_gtype=="BMFU"||tmp_sel_gtype=="TTFU"){
			//if(tmp_sel_gtype == "BU"){
				tmp = "./"+tmp_sel_gtype+"_future/body_var.php";
...
if (top.keep_LoveI_array_VB==undefined) top.keep_LoveI_array_VB=new Array();
...
function addShowLoveI(gid,getDateTime,getLid,team_h,team_c){
	...
	if(sw){
		...
		if(rtype!="re"){
			loveI_has_in=true;
			for (i=0;i < top.keep_LoveI_array_VB.length;i++){
				if(top.keep_LoveI_array_VB[i][0]==gid){
					loveI_has_in=false;
					break;
				}
			}
			if(loveI_has_in){
				...
				top.keep_LoveI_array_VB.push(new Array(gid,tmpDateTime,getLid,team_h,team_c));
			}
		}
		chkOKshowLoveI();
	}
...
function auto_re_addShowLoveI(Game_Data){
	...
	//}
	for (var i=top.keep_LoveI_array_VB.length-1;i >= 0;i--){
		var tmp=top.keep_LoveI_array_VB[i][1].split("<br>");
		newTime=change_time(tmp[1])+":00";
		var tmp_today_gmt=top.today_gmt.split("-");
		chk_date_time=tmp_today_gmt[0]+"-"+tmp[0]+" "+newTime;
		var tmp_find=false;
		var tmp_gid=top.keep_LoveI_array_VB[i][0];
		if(chk_date_time < top.today_gmt+" "+top.now_gmt){
			...
			if(tmp_find){
				top.ShowLoveIarray[getGtype] = arraySort(top.ShowLoveIarray[getGtype] ,top.keep_LoveI_array_VB[i]);
			}
			//tmpAry.splice(i,1);
			Array.prototype.splice.call(top.keep_LoveI_array_VB,i,1);
...
function chkDelshowLoveI(data2,gid){
	...
	for (var i=0 ; i < tmp.length ; i++){
		...
		if(tmpdata1 == tmpdata){
			...
			//}
			//alert(data);
			for (var s=0;s < top.keep_LoveI_array_VB.length;s++){
				//alert(top.keep_LoveI_array_VB[s][0]+"---"+gid);
				if(top.keep_LoveI_array_VB[s][0]==gid) Array.prototype.splice.call(top.keep_LoveI_array_VB,s,1);
			}
			//top.keep_LoveI_array_VB=tmpAry;
			continue;
		}
...
function MarketPeriod(){
	try{
		...
		spanM.innerHTML = top["str_VB_Market_"+Market];
		spanP.innerHTML = top["str_VB_Period_"+Period];
	}catch(e){
...
function chgMarket(event){//Main,All
	...
	tmp = tmp.substring(1,tmp.length).split("l")
	tmp = tmp[0];
	top.CM.set("VBRtype@"+tmp,rtype.split("_")[1]);
	//日期特早 切換帶回全部
