...
function onLoad(){
	if (""+eval(mother+sel_gtype+"_lid_ary_RE")=="undefined") eval(mother+sel_gtype+"_lid_ary_RE='ALL'");
	var len =lid_form.elements.length;
	parent.setleghi(document.body.scrollHeight);
	if(eval(mother+sel_gtype+"_lid_ary_RE")=='ALL'){
		...
		for (var i = 1; i < len; i++) {
			...
			if(e.id.substr(0,3)=="LID"&&e.type=='checkbox') {
				if(eval(mother+sel_gtype+"_lid_ary_RE").indexOf(e.id.substr(3,e.id.length)+"|",0)!=-1){
					e.checked='true';
				}
			}
		}		
...
function chk_league(){
	...
	var gcount=0;
  	if(lid_form.sall.checked) {
  		eval("top."+sel_gtype+"_lid['"+sel_gtype+"_lid_ary_RE']="+mother+sel_gtype+"_lid_ary_RE='ALL'");
  		eval("top."+sel_gtype+"_lid['"+sel_gtype+"_lname_ary_RE']="+mother+sel_gtype+"_lname_ary_RE='ALL'");
  	}else{
		for (var i = 1; i < len; i++) {
			var e = lid_form.elements[i];
	...
	if(tmp==""){
		...
		if(gcount>0){
			eval("top."+sel_gtype+"_lid['"+sel_gtype+"_lid_ary_RE']="+mother+sel_gtype+"_lid_ary_RE=strlid");
			eval("top."+sel_gtype+"_lid['"+sel_gtype+"_lname_ary_RE']="+mother+sel_gtype+"_lname_ary_RE=strlname");
		}else{
			eval("top."+sel_gtype+"_lid['"+sel_gtype+"_lid_ary_RE']="+mother+sel_gtype+"_lid_ary_RE='ALL'");
			eval("top."+sel_gtype+"_lid['"+sel_gtype+"_lname_ary_RE']="+mother+sel_gtype+"_lname_ary_RE='ALL'");
		}	
