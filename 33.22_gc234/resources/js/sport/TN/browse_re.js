...
var pg=0;
var sel_league='';
var uid='505i3zacwim13706722l868450';
var loading = 'Y';
var loading_var = 'Y';
var ShowType = '';
var ltype = '3';
...
function ShowGameList(){
	parent.header.chg_button_bg('TN','rb');
...
function getLayer(onelayer,gamerec,odd_f_type){
 ...
 if (top.hot_game==""){	
	...
	if (ObjDataFT[gamerec].isMaster=="Y"||ObjDataFT[gamerec].isMaster!="N"){
			onelayer=onelayer.replace("*NOWSERVER*","");
			onelayer=onelayer.replace("*NODATA*","");
			
			if(ObjDataFT[gamerec].nowServer==0){
				onelayer=onelayer.replace("*NOWC*","class=\'tn_dot_on\'");
			}
			if(ObjDataFT[gamerec].nowServer==1){
				onelayer=onelayer.replace("*NOWH*","class=\'tn_dot_on\'");
			}
			onelayer=onelayer.replace("*NOWH*","class=\'tn_dot_out\'");
			onelayer=onelayer.replace("*NOWC*","class=\'tn_dot_out\'");
			onelayer=onelayer.replace("*GAMEH*",ObjDataFT[gamerec].scoreGameH);
			onelayer=onelayer.replace("*GAMEC*",ObjDataFT[gamerec].scoreGameC);
			onelayer=onelayer.replace("*SETH*",ObjDataFT[gamerec].scoreSetH);
			onelayer=onelayer.replace("*SETC*",ObjDataFT[gamerec].scoreSetC);
			onelayer=onelayer.replace("*POINTH*",(ObjDataFT[gamerec].scorePointH =='Adv'?'A':ObjDataFT[gamerec].scorePointH));
			onelayer=onelayer.replace("*POINTC*",(ObjDataFT[gamerec].scorePointC =='Adv'?'A':ObjDataFT[gamerec].scorePointC));
			//20150529 bill 補上天氣延賽
			if (ObjDataFT[gamerec].showdelay=="Y") {
				onelayer=onelayer.replace("*SHOWDELAY*","");	
			}else{
				onelayer=onelayer.replace("*SHOWDELAY*","style='display:none;'");
			}
			
	}else{
			onelayer=onelayer.replace("*NOWSERVER*"," style='display: none;'");
			onelayer=onelayer.replace("*NODATA*","&nbsp;");
			//20150529 bill 補上天氣延賽
			onelayer=onelayer.replace("*SHOWDELAY*","style='display:none;'");
	}
	...
	/* 2014-10-27 mordecai useless
	// 若場次為主隊局數或客隊局數 只秀單邊隊伍名稱
	if(ObjDataFT[gamerec].gnum_h.substring(0,1)=="7"){
		onelayer=onelayer.replace("*TEAM_H*",ObjDataFT[gamerec].team_h.replace("[Mid]","").replace("[中]",""));
		onelayer=onelayer.replace("*TEAM_C*","");
	}else if(ObjDataFT[gamerec].gnum_h.substring(0,1)=="8"){
		onelayer=onelayer.replace("*TEAM_H*",ObjDataFT[gamerec].team_c);
		onelayer=onelayer.replace("*TEAM_C*","");
	}else{*/
	...
	//全場
	//獨贏
	onelayer=onelayer.replace("*RATIO_MH*",parseUrl(uid,odd_f_type,"H",ObjDataFT[gamerec],gamerec,"M"));/*獨贏賠率*/
	...
	//onelayer = onelayer.replace("*MORE*",parseAllBets(ObjDataFT[gamerec]));  //2014.08網球多玩法
	...
	if (ObjDataFT[gamerec].eventid != "" && ObjDataFT[gamerec].eventid != "null" && ObjDataFT[gamerec].eventid != undefined) {	//判斷是否有轉播
		tmpStr= VideoFun(ObjDataFT[gamerec].eventid, ObjDataFT[gamerec].hot, ObjDataFT[gamerec].play, "TN");
...
function parseUrl(uid,odd_f_type,betTeam,GameData,gamerec,wtype){
	...
	urlArray['R'] = new Array("../TN_order/TN_order_re.php",eval("GameData.team_"+betTeam.toLowerCase()));
	urlArray['OU'] = new Array("../TN_order/TN_order_rou.php",(betTeam=="C"?top.strOver:top.strUnder));
	urlArray['M'] = new Array("../TN_order/TN_order_rm.php",(betTeam=="N"?top.str_irish_kiss:eval("GameData.team_"+betTeam.toLowerCase())));
	urlArray['EO'] = new Array("../TN_order/TN_order_rt.php", (betTeam=="O"?top.str_o:top.str_e));
	urlArray['OUH'] = new Array("../TN_order/TN_order_rouhc.php",(betTeam=="O"?top.strOver:top.strUnder));
	urlArray['OUC'] = new Array("../TN_order/TN_order_rouhc.php",(betTeam=="O"?top.strOver:top.strUnder));
	...
	var team = urlArray[wtype][1].replace("[Mid]","[N]").replace("<font color=gray>","").replace("</font>","");
	...
	var ret = "<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('TN','"+rewtype[wtype]+"','"+param+"');\" title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+">"+ioratio+"</font></a>";
...
function getParam(uid,odd_f_type,betTeam,wtype,GameData){
	...
	for(var i=0; i<paramArray[wtype].length; i++){
		if(i > 0)	param+="&";
		if(paramArray[wtype][i] == "wtype"){
			param+=paramArray[wtype][i]+"=R"+eval(paramArray[wtype][i]);
		}else{
...
function parsemore(GameData){
	var ret = "";
	if(GameData.more=="0"){
		ret = "&nbsp;";
	}else{
		ret = "<A href=javascript: onClick=parent.show_more('"+GameData.gid+"',event);>"+"<font class='total_color'>+"+GameData.more+"&nbsp;</font>"+top.str_more+"</A>";
	}
	return ret;
}
...
//2014.08網球多玩法
function parseAllBets(GameData){
	var ret="";
	if(GameData.more=='0'){
		ret="";
	}else{
		var datetime = GameData.datetime.split("<br>");
	 	ret="<A href=javascript: class='tn_more' onClick=show_allbets('"+GameData.gid+"',event,'"+datetime[0]+"');><font class='total_color'>"+top.str_all_bets+" ("+GameData.more+")</font></A>";
	}			
	return ret;	
}
...
function show_allbets(gid,evt,datetime){
	...
	body_browse.document.getElementById('MTN').className="more_bar";
	...
	var  url="body_var_re_allbets.php?gid="+gid+"&uid="+uid+"&ltype="+ltype+"&langx="+top.langx+"&gtype=TN&showtype=RB&date="+datetime;
	body_browse.showdata.location.href = url;
}
...
function parseMyLove(GameData){
	...
	//if (top.casino == "SI2") {
		...
		tmpStr+= VideoFun(GameData.eventid, GameData.hot, GameData.play, "TN");
		}
	//}
	tmpStr+= "</td>";
	tmpStr+= "</tr></table>";
...
function onLoad(){
	...
	if(sel_gtype=="TU"){
		...
		if (rtype.split("_")[0]=="r" || rtype=="pd" || rtype == "p3"){
...
function showDateSel(){
	...
	for(var i=0; i<dateLength; i++){
		...
		if(i == 0){
			...
			tmp = tmp.replace("*DATE_SHOWTYPE*","TU");
		...
		if(top.sel_gd=="today" && i==0){
			...
			if((tAry.length > 1) && ((i+1) == dateLength)){
				sel_class = "day_text_red";
...
//	conscroll.height=screen.availHeight;
	loading_var = 'Y';
	if(Level=="up"){
		var tmp = "./"+sel_gtype+"_browse/body_var.php";
		if (sel_gtype=="TU"){
			tmp = "./TN_future/body_var.php";
		}
	}else{
	...
	if(top.showtype=='hgft'&&sel_gtype=="TU"){
		l_id=3;	
	}
	if(rtype == "pr") top.hot_game="";
	var homepage = tmp+"?uid="+uid+"&rtype="+rtype+"&langx="+langx+"&mtype="+mtype+"&page_no="+pg+"&league_id="+l_id+"&hot_game="+top.hot_game;
	//alert("g_date==>"+g_date)
	if (sel_gtype=="TU"){
...
if (top.keep_LoveI_array_TN==undefined) top.keep_LoveI_array_TN=new Array();
...
function addShowLoveI(gid,getDateTime,getLid,team_h,team_c){
	...
	if(sw){
		...
		if(rtype!="re"){
			loveI_has_in=true;
			for (i=0;i < top.keep_LoveI_array_TN.length;i++){
				if(top.keep_LoveI_array_TN[i][0]==gid){
					loveI_has_in=false;
					break;
				}
			}
			if(loveI_has_in){
				...
				top.keep_LoveI_array_TN.push(new Array(gid,tmpDateTime,getLid,team_h,team_c));
			}
		}
		chkOKshowLoveI();
	}
...
function auto_re_addShowLoveI(Game_Data){
	...
	for (var k=0;k < top.keep_LoveI_array_TN.length;k++){	
		tmpAry[tmpAry.length] = top.keep_LoveI_array_TN[k];
	}
	*/
	for (var i=top.keep_LoveI_array_TN.length-1;i >= 0;i--){
		var tmp=top.keep_LoveI_array_TN[i][1].split("<br>");
		newTime=change_time(tmp[1])+":00";
		var tmp_today_gmt=top.today_gmt.split("-");
		chk_date_time=tmp_today_gmt[0]+"-"+tmp[0]+" "+newTime;
		var tmp_find=false;
		var tmp_gid=top.keep_LoveI_array_TN[i][0];
		if(chk_date_time < top.today_gmt+" "+top.now_gmt){
			...
			if(tmp_find){
				top.ShowLoveIarray[getGtype] = arraySort(top.ShowLoveIarray[getGtype] ,top.keep_LoveI_array_TN[i]);
			}
			//tmpAry.splice(i,1);
			Array.prototype.splice.call(top.keep_LoveI_array_TN,i,1);
			
		}
	}
	chkOKshowLoveI();
	
	//top.keep_LoveI_array_TN = tmpAry;
}
...
function chkDelshowLoveI(data2,gid){
	...
	for (var i=0 ; i < tmp.length ; i++){
		...
		if(tmpdata1 == tmpdata){
			...
			*/
			for (s=0;s < top.keep_LoveI_array_TN.length;s++){
				if(top.keep_LoveI_array_TN[s][0]==gid) Array.prototype.splice.call(top.keep_LoveI_array_TN,s,1);//tmpAry.splice(s,1);
			}
			//top.keep_LoveI_array_TN=tmpAry;
			continue;
		}
...
function setMarketPeriod(aRtype){
	...
	else if(aRtype=="p3" ){Market="Main";Period="Hide";}
}
...
function MarketPeriod(){
	try{
		...
		spanM.innerHTML = top["str_TN_Market_"+Market];
		spanP.innerHTML = top["str_TN_Period_"+Period];
	}catch(e){
...
function chgMarket(event){//Main,All
	...
	tmp =	tmp.substring(1,tmp.length).split("l")
	tmp = tmp[0];
	top.CM.set("TNRtype@"+tmp,rtype.split("_")[1]);
//	try{
//		if(g_date==1 && rtype != "r_no")g_date="ALL";
//	}catch(e){
//		console.log("g_date_error in chgMarket");
//	}
	refreshReload();
}
