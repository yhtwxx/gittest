...
<c:choose>
    ...
    <table class="MMain" border="1" style="margin-bottom: 8px;">
 		<tr>
            <td width="100%" colspan="2" align="center"><h2><spring:message code="public.set" /><spring:message code="public.cashPassword" /></h2></td>
        </tr>
        <tr>
            <td width="100%" colspan="2" align="center"><spring:message code="public.username" />: ${accountno }</td>
        </tr>
        <tr>
            <td width="41%" class="" align="right"><spring:message code="public.cashPassword" />:</td>
            <td class="" align="left">
            <input type="password" name="newpassword" id="newpassword" /></td>
        </tr>
        <tr>
            <td width="41%" class="" align="right"><spring:message code="public.confirmPassword" />:</td>
            <td class="" align="left">
        ...
        <tr>
            ...
            <td>
            <div class="Next_btn">
            <a href="javascript:history.go(-1)" class="nui-btn nui-btn-theme-default">
            <spring:message code="public.return" /></a> 
            <a href="javascript:void(0);" onclick="submit();" class="nui-btn nui-btn-theme-main">
            <spring:message code="public.Submit" /></a></div>
            </td>
         </tr>
    </table>
    </c:when>
	...
	<table class="MMain" border="1" style="margin-bottom: 8px;">
 		<tr>
            <td width="100%" colspan="2" align="center"><h2><spring:message code="public.member.ModifyCashPassword" /></h2></td>
        </tr>
        <tr>
            <td width="100%" colspan="2" align="center"><spring:message code="public.username" />: ${accountno }</td>
        </tr>
...
<c:choose>
    ...
    function submit(){
			...
			if (newpassword==null || newpassword == "") {
				layer.msg(commonlangpackage['PleaseEnterPassword'], 1, 0);
                $("#newpassword").focus();
                return false;
            }
			var newpassword2 = $("#newpassword2").val();
			if (newpassword2==null || newpassword2 == "") {
				 layer.msg(commonlangpackage['ConfirmPassword'], 1, 0);
                $("#newpassword2").focus();
                return false;
            }
			if(newpassword2!=newpassword){
				layer.msg(commonlangpackage['ConfirmPasswordORPasswordNotCorrect'], 1, 0);
                $("#newpassword2").focus();
                return false;
			}
			var url="&cashPasswd="+newpassword2;
            $.ajax({url: "../user/updateuserDataJson.do?timeStamp=" + new Date().getTime()+url,type: "post",dataType: "json",
                success: function(b) {
                    if(0 == b.code){
                    	layer.msg(commonlangpackage['PwdSetSuccess'], 1, 0);
                    	setTimeout(
                                function(){
    ...
    function submit(){
            ...
            $.ajax({url: "../user/updatePasswordJson.do?timeStamp=" + new Date().getTime()+"&pwdtype=2&oldpassword="+oldpassword+"&newpassword="+newpassword,type: "get",dataType: "json",
                success: function(b) {
                    if(0 == b.code){
                    	layer.msg(commonlangpackage['WithdrawalPwdModificationSuccess'], 1, 0);
                    	setTimeout(
                                function(){
                               	 window.location.href="${ctx}/user/userPassword.do";
                                },1000
