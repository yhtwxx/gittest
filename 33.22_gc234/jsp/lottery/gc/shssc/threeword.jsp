...
<script>
	$(function() {
		//循环输出
		$.ajax({
			url : "${ctx}/shssc/itemDate.do?timeStamp=" + new Date().getTime() + "&poolId=6",
			type : "get",
			dataType : "json",
			success : function(b) {
				if (0 == b.code) {
		...
		// 下注金额栏位
		ui._$view_inputs = $("#kuaijiexiaju_input").on('keyup', function() {
			var el = this;
		...
		// 头 radio 输入栏位
		ui.$headRadio = ui._$view.find("input[name*=head]:radio").change(
				function() {
					ui.$headInput.val(getTypeItems($(this).val()));
				});

		// 中 radio 输入栏位
		ui.$middleRadio = ui._$view.find("input[name*=middle]:radio").change(
				function() {
					ui.$middleInput.val(getTypeItems($(this).val()));
				});

		// 尾 radio 输入栏位
		ui.$tailRadio = ui._$view.find("input[name*=tail]:radio").change(
				function() {
					ui.$tailInput.val(getTypeItems($(this).val()));
				});

		// 和尾 radio 输入栏位
		ui.$tailSumRadio = ui._$view.find("input[name*=sumTail]:radio").change(
				function() {
		...
		var render = function() {
			...
			if (cHead.length || cMiddle.length || cTail.length
					|| cTailSum.length || cSum.length || cDa.length
					|| cOptions.length) {

				if (ui._$parent.data('tabs-active')) {
					$content = ui._$parent.data('tabs_data').views[ui._$parent
							.data('tabs-active')];
				} else {
					$content = ui._$parent;
				}
				...
				var allGold = 0;
				$("table[" + this_tabs + "]").find('.choose-num').each(
						function(i) {
							...
							if (cHead.length > 0
									&& $.inArray(num.charAt(0), cHead) < 0) {
								return;
							}

							if (cMiddle.length > 0
									&& $.inArray(num.charAt(1), cMiddle) < 0) {
								return;
							}

							if (cTail.length > 0
									&& $.inArray(num.charAt(2), cTail) < 0) {
								return;
							}
							...
							if (cOptions.length > 0) {
								...
								$.each(cOptions, function(k, v) {
									isCheck = (isCheck && config_options[v]
											.hasOwnProperty(num));
								});
							...
							$el.addClass('active').next().find('input').val(
									gold);
						});
		...
		function count() {
			...
			var nums = $("#locate-box").find(".itemCode").filter(function() {
				return this.value !== "";
			}).size();
